...
from conflict_task.util.dictionary import get_or_fatal_exit, get_type, get_type_or_fatal_exit
...
class Instructions:
    ...
    def _parse_instruction_settings(self, instruction_settings):
        self.key_forward = get_type_or_fatal_exit(instruction_settings, "key_forward", str, "Please indicate forward key")
        
        self.allow_backward = get_type(instruction_settings, "allow_backward", bool, True)
        
        if self.allow_backward:
            self.key_backward = get_type_or_fatal_exit(instruction_settings, "key_backward", str, "Please indicate backward key")

        instructions = get_or_fatal_exit(instruction_settings, "screens", "Please include some screens in instructions")

        response = {
            "response": {
                "keys": [self.key_forward]
            }
        }
