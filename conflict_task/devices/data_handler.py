...
class DataHandler:
    ...
    def __init__(self, experiment_name: str, subject_info: dict = {'participant': '', 'session': '001'}):
    ...
    def start_participant_data(self):
        ...
        dlg = gui.DlgFromDict(self.subject_info, sortKeys=False, title=self.experiment_name)
        ...
        self.filename = thisDir + os.sep + "data" + os.sep + f"{self.subject_info['participant']}_{self.experiment_name}_{self.subject_info['date']}"

        self._data_handler = data.ExperimentHandler(name=self.experiment_name, version=version, extraInfo=self.subject_info, saveWideText=True, dataFileName=self.filename)
