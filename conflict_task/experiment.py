...
from conflict_task.devices import Window, DataHandler, input_device
...
class Experiment:
    ...
    def __init__(self, trial_values: list, experiment_settings: dict) -> None:
        ...
        # Sanity check
        test_or_fatal_exit(isinstance(trial_values, list), "Trial values must be a list of dictionaries")
        test_or_fatal_exit(isinstance(experiment_settings, dict), "Experiment settings must be a dictionary")
    ...
    def _parse_experiment_settings(self, experiment_settings: dict) -> None:
        ...
        # Experiment settings
        # -----------------------------------------------
        self.name = get_type_or_fatal_exit(experiment_settings, "name", str, "Please specify experiment 'name'")
        ...
        # Input device
        # -----------------------------------------------
        input_device_class = get_type(experiment_settings, "input_device", str, "Keyboard")
        test_or_fatal_exit(hasattr(input_device, input_device_class), f"Input device specified in 'input_device' does not exist. No input device named {input_device_class}")
        ...
        experiment_sequence: dict = get_type_or_fatal_exit(
            experiment_settings, "experiment_sequence", dict,
        ...
        block_settings: dict = get_type_or_fatal_exit(
            experiment_sequence, "block", dict,
            "Experiment sequence must have 'block' settings"
        )

        self.nr_blocks = get_type_or_fatal_exit(block_settings, "nr_blocks", int, "'nr_blocks' must be specified")
        self.nr_trials = get_type_or_fatal_exit(block_settings, "nr_trials", int, "'nr_trials' must be specified")

        if trial_settings := get_type_or_fatal_exit(block_settings, "trial", dict, "'trial' settings must be specified"):
        ...
        if self.block_trial.takes_trial_values:
            test_or_fatal_exit(len(self.trial_values) == self.nr_blocks,
                f"Number of blocks in trial values not corresponding to 'nr_blocks' - {len(self.trial_values)} != {self.nr_blocks}")
            test_or_fatal_exit(all(len(trials) == self.nr_trials for trials in self.trial_values),
    ...
    def _create_sequence(self, sequence_settings: dict) -> sequence.Sequence:
        ...
        test_or_fatal_exit(isinstance(sequence, sequence_type), f"Sequence type specified in 'type' does not exist. No sequence named {sequence_type}")
        return getattr(sequence, sequence_type)(self.window, self.input_device, sequence_settings)
    ...
    def run(self, debug_data = False):
        ...
        for pre in self.pre:
            ...
            self.data_handler.add_data_dict_and_next_entry(experiment_data | pre.get_data())
        ...
        for block in range(self.nr_blocks):
            block_data = experiment_data | {
                    "block": block + 1
            }

            # BETWEEN BLOCK
            if block: # Skip first block
                continue_experiment = self.between_blocks.run(allow_escape=self.allow_escape)

                self.data_handler.add_data_dict_and_next_entry(block_data | self.between_blocks.get_data())
            ...
            for trial in range(self.nr_trials):
                ...
                self.data_handler.add_data_dict_and_next_entry(self.block_trial.get_data())
