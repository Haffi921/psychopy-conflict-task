...
def counterbalance(
    ...
    def check_for_parameter_errors():
        ...
        if levels < 1:
            ...
            Warning("Increased levels could make the algorithm unsolvable. Currently it has only been tested on 'level' = 1 or 2")
    ...
    if not Force:
        for i in base_matrix.flat:
            if i % 1 != 0:
                raise RuntimeError(f"Number of trials not a whole number product of counterbalancing conditions: {trials} / {base_matrix.size} = {trials / base_matrix.size}")

    if verbose:
        print(f"Creating a counterbalanced {trials}-trial sequence")
        if alternating:
            print(f"Using {length} possible conditions in {dimensions} dimensions, alternating between {alternating_levels} instances")
            print(f"Comes down to {(length**dimensions) * alternating_levels} counterbalancing conditions")
        else:
